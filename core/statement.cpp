...
//      std::cerr << __PRETTY_FUNCTION__ << " 1 " << std::endl;
    if (mInRow != 0) {
        mInRow->Release();
        mInRow = 0;
    }
//              std::cerr << __PRETTY_FUNCTION__ << " 2a " <<std::endl;
    if (mOutRow != 0) {
        mOutRow->Release();
        mOutRow = 0;
    }
//          std::cerr << __PRETTY_FUNCTION__ << " 2b " <<std::endl;
    mResultSetAvailable = false;
    mCursorOpened = false;
...
{
//     std::cerr << "0" << __PRETTY_FUNCTION__ << std::endl;    
    if (database == 0)
    ...
    mDatabase = database;
//     std::cerr << "1" << __PRETTY_FUNCTION__ << database <<std::endl;
    mDatabase->AttachStatementImpl(this);
}

void StatementImpl::DetachDatabaseImpl()
{
//      std::cerr << __PRETTY_FUNCTION__ << "1 mDatabase" <<mDatabase<< std::endl;
    if (mDatabase == 0) return;
//      std::cerr << __PRETTY_FUNCTION__ << "1a " << std::endl;
//      try {
    if (mType != IBPP::stUnknown)
        this->Close();
//     this->Close();
//     } catch (...) { }
//      std::cerr << __PRETTY_FUNCTION__ << "2 " <<std::endl;
...
{
//          std::cerr << "0" << __PRETTY_FUNCTION__ <<"mTransaction"<<mTransaction<< std::endl;   

    if (mTransaction == 0) return;
//  std::cerr << __PRETTY_FUNCTION__ << "" <<std::endl;
     if (mType != IBPP::stUnknown) Close();
...
//        std::cerr << __PRETTY_FUNCTION__ << std::endl;     
    ...
    try {
//             std::cerr <<"1 " <<__PRETTY_FUNCTION__ << std::endl;       
        if (mTransaction != 0) mTransaction->DetachStatementImpl(this);
//             std::cerr <<"2 " <<__PRETTY_FUNCTION__ << std::endl;     
        
    }
    catch (...) { }
    try {
//              std::cerr <<"3 " <<__PRETTY_FUNCTION__ << std::endl;           
        if (mDatabase != 0) mDatabase->DetachStatementImpl(this);
//             std::cerr <<"4 " <<__PRETTY_FUNCTION__ << std::endl;    
