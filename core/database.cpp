...
//     std::cerr << __PRETTY_FUNCTION__ << " mStatements size " <<mStatements.size() <<std::endl;
    while ( mStatements.size() > 0 )
    {
        StatementImpl* p =  mStatements.back();
//         std::cerr << __PRETTY_FUNCTION__ << " StatementImpl  "<<std::endl;
        p->DetachDatabaseImpl();
        this->DetachStatementImpl(p);

    }
//     std::cerr << __PRETTY_FUNCTION__ << "5 " <<std::endl;
    // Let's detach from all Transactions
    while ( mTransactions.size() > 0 )
        mTransactions.back()->DetachDatabaseImpl ( this );
//     std::cerr << __PRETTY_FUNCTION__ << "6 " <<std::endl;
    // Let's detach from all Events
    while ( mEvents.size() > 0 )
        mEvents.back()->DetachDatabaseImpl();
//     std::cerr << __PRETTY_FUNCTION__ << "7 " <<std::endl;
...
void DatabaseImpl::DetachStatementImpl ( StatementImpl* st )
{
    ...
    std::vector< ibpp_internals::StatementImpl* >::iterator it = std::find ( mStatements.begin(), mStatements.end(), st );
    if (it != mStatements.end() )
    mStatements.erase ( it );      
